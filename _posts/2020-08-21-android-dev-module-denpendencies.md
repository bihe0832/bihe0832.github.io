...
为了方便组件依赖的管理，我们对整个组件化项目的工程配置、编译、依赖管理等进行了整体重构。

<img src="./../public/images/android_dev/aaf_dependencies.png" width="80%" />


包括**模块怎么导入、依赖关系怎么添加、修改以后怎么更新组件**。由于Android 原有的构建方式解决这几个问题比较复杂，因此我们基于Gradle对于这部分做了比较大的改动。重构以后：
